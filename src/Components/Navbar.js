...
function Navbar() {
    ...
    const { currentUser, logout } = useContext(AuthContext); // Récupérer l'état actuel de l'utilisateur et la fonction logout

    const items1 = [
        { text: 'Todo List', icon: 'fas fa-home' },
        { text: 'Calendar', icon: 'fas fa-info' },
        { text: 'Reminders', icon: 'fas fa-brain' },
        { text: 'Planning', icon: 'fas fa-book' },
    ];

    const items2 = [
        { text: 'History', icon: 'fas fa-briefcase' },
        { text: 'Our Team', icon: 'fas fa-building' },
        { text: 'Blog', icon: 'fas fa-users' },
    ];
    ...
    const goToProfile = () => {
        navigate('/profile'); // Assurez-vous d'avoir une page de profil
    };

    const handleLogout = async () => {
        try {
            await logout(); // Appeler la fonction logout du contexte
            navigate('/'); // Rediriger vers la page d'acceuil après déconnexion
        } catch (error) {
    ...
    return (
        <div>
            <header>
                ...
                <nav ref={navRef}>
                    <Dropdown buttonTex="Features" content={(
                        <>
                            {items1.map(item => (
                                <DropDownItem key={item.text} iconClass={item.icon}>
                                    {item.text}
                                </DropDownItem>
                            ))}
                        </>
                    )} />
                    <Dropdown buttonTex="Company" content={(
                        <>
                            {items2.map(item => (
                                <DropDownItem key={item.text} iconClass={item.icon}>
                                    {item.text}
                    ...
                    {currentUser ? (
                        <div className="user-dropdown">
                            <Dropdown buttonTex={<FaUser />} content={(
                                <>
                        ...
                        <>
                            <button className="btn success" onClick={goToLogin}>Login</button>
                            <button className="btn success" onClick={goToRegister}>Register</button>
                        </>
                    )}
                </nav>
