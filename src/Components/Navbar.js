...
function Navbar() {
    const navRef = useRef();
    const items1 = ["Todo List","Calendar","Reminders","Planning"];
    const items2 = ["History","Our Team","Blog"];
        ...
        <div>
            <header>
                    ...
                    <Dropdown buttonTex="Features" content={<>{
                       items1.map(item => <DropDownItem key={item}>{` ${item}`}</DropDownItem>)
                    }</>}/>
                     <Dropdown buttonTex="Company" content={<>{
                       items2.map(item => <DropDownItem key={item}>{` ${item}`}</DropDownItem>)
                    }</>}/>
                <nav ref={navRef}>
                    <a href="/#">Careers</a>
                    <a href="/#">About</a>
